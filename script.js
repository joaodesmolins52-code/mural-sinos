...
joinForm.addEventListener("submit",async e=>{e.preventDefault(); if(!supabaseReady){toast("Configure o Supabase primeiro ou use o modo local.");return;} const code=$("#joinCode").value.trim().toUpperCase(), name=$("#joinName").value.trim()||"Jogador"; try{ const {data,error}=await supabase.rpc("join_campaign",{campaign_code:code,display_name:name}); if(error) throw error; const row=data?.[0]; if(!row)throw new Error("CAMPAIGN_NOT_FOUND"); campaignId=row.campaign_id; campaignCode=code; playerName=name; playerRole=row.player_role; localStorage.setItem(localKeys.session,JSON.stringify({campaignId,campaignCode,playerName,playerRole})); await loadCampaignData(); closeSetup(); subscribeRealtime(); toast(`Você entrou em ${row.campaign_name}.`);}catch(err){console.error(err);toast("Não encontrei essa mesa ou o código está errado.");}});
createForm.addEventListener("submit",async e=>{e.preventDefault(); if(!supabaseReady){toast("Configure o Supabase primeiro.");return;} const name=$("#createName").value.trim()||"O Som que Não Deveria Existir", player=$("#createPlayerName").value.trim()||"Mestre"; try{const {data,error}=await supabase.rpc("create_campaign",{campaign_name:name,display_name:player}); if(error)throw error; const row=data?.[0]; if(!row)throw new Error("CREATE_FAILED"); campaignId=row.campaign_id; campaignCode=row.campaign_code; playerName=player; playerRole="master"; localStorage.setItem(localKeys.session,JSON.stringify({campaignId,campaignCode,playerName,playerRole})); await loadCampaignData(); closeSetup(); subscribeRealtime(); toast(`Mesa criada. Código: ${campaignCode}`); $("#setupResult").hidden=false; $("#setupResult").textContent=`Código da mesa: ${campaignCode}`;}catch(err){console.error(err);toast("Não foi possível criar a mesa. Rode o supabase.sql antes.");}});
